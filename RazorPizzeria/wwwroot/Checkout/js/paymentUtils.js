	...
	this.getPaymentSummary = function() {
		...
		selectedProducts.forEach(function(product) {
			...
			if (window.CardForm) {
				...
				var desc = document.querySelector('#product-name-description-' + inputId).innerText;
				var price = '';
			...
			if (window.CardForm) {
				...
				} else if (!imageArea && document.querySelector('[pid="' + pid + '"].form-product-item img')) {
					// old UI
			...
			// CALCULATE QUANTITY
			var quantity = 0;
			// if there is no subproduct
			var baseSubLabelContainerEl = document.querySelector('[pid="' + pid + '"].form-product-item .form-sub-label-container');
			// if there is subproduct
			var hasSubProduct = document.querySelector('[pid="' + pid + '"].form-product-item .form-product-has-subproducts');
			if (baseSubLabelContainerEl) {
				if (document.querySelector('input[id*="_quantity_'+ pid +'"]')) {
					// if input quantity
					quantity = document.querySelector('input[id*="_quantity_'+ pid +'"]').value;
				} else if (document.querySelector('select[id*="_quantity_'+ pid +'"]')) {
					// if dropdown quantity
					quantity = document.querySelector('select[id*="_quantity_'+ pid +'"]').value;
				} else {
					quantity = 1;
				}
			} else if (hasSubProduct) {
				// if there is subproduct
				var subProductInputs = document.querySelectorAll('[pid="' + pid + '"].form-product-item .form-product-container #' + inputId + '_subproducts .form-subproduct-quantity');
				subProductInputs.forEach(function(input) {
			...
			// TODO :: check if symbol should be beforehand or after
			var filteredProduct = JotForm.pricingInformations.items.filter(function(item) { return item.productID === pid; })[0]
			if (filteredProduct) {
				price = filteredProduct.total_amount;
			}

			// CALCULATE PRICE WHEN SUBPRODUCT ENABLED
			var filteredSubProductsForPrice = JotForm.pricingInformations.items.filter(function(item) { return item.productID.length !== '4' && item.finalProductID.split('_')[0] === pid; });
			if (hasSubProduct && filteredSubProductsForPrice) {
				price = 0;
				filteredSubProductsForPrice.forEach(function(item) {
